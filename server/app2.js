...
app.get('/getBoardDetails/:userId/:boardId', async (req, res) => {
    try {
      ...
      // Dapatkan detail board dan semua task yang terkait dengan board tersebut
      const boardDetails = await models.Boards.findOne({
        where: { BoardId: boardId },
        include: [
          {
            model: models.Tasks,
            as: "Tasks", // Tambahkan ini untuk menyesuaikan dengan alias yang diberikan pada asosiasi
            include: [
              {
                model: models.Users,
                as: "UserAssign",
